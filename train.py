...
def train(rank=0, args=None, temp_dir="", replay_buffer=None):
    ...
    if distributed:
        ...
        value_function = DDP(_mlp_net, device_ids=[local_rank, ])
        policy = DDP(_policy, device_ids=[local_rank, ])
        ...
        value_function = _mlp_net.to(train_device)
        policy = _policy.to(train_device)
    ...
    optimizer = Adam(model.parameters(), lr=train_config.model_lr, betas=(train_config.beta1, train_config.beta2))
    optimizer_pi = Adam(policy.parameters(), lr=train_config.policy_lr, betas=(train_config.beta1, train_config.beta2))
    optimizer_v = Adam(value_function.parameters(), lr=train_config.value_lr, betas=(train_config.beta1, train_config.beta2))
    ...
    scheduler_v = lr_scheduler.LambdaLR(
        optimizer_v, lr_lambda=lambda t: min((t + 1) / train_config.warmup_v, 1.0)
    ) if train_config.warmup_v > 0 else None
    
    scheduler_pi = lr_scheduler.LambdaLR(
        optimizer_pi, lr_lambda=lambda t: min((t + 1) / train_config.warmup_pi, 1.0)
    ) if train_config.warmup_pi > 0 else None
    ...
    if is_leader:
        ...
        hps = {
            ...
            "model": model_config,
            "value_func": ValueNetwork_config,
            "train": train_config
        }
    ...
    trainer = Trainer(
        ...
        alg=train_config.alg,
        use_baseline=train_config.use_baseline,
        max_buffer_length=train_config.max_buffer_length,
    ...
    if is_leader:
        trainer.train(evaluator, chkpt_path=chkpt_path, image_dir=image_dir, logger=wandb)
    else:
...
def main():
    ...
    parser.add_argument("--value_lr", default=0.0002, type=float, help="learning rate")
    parser.add_argument("--policy_lr", default=1e-3, type=float, help="learning rate")
    ...
    parser.add_argument("--image_intv", default=10, type=int)
    parser.add_argument("--num-samples", default=66, type=int, help="number of images to sample and save")
    ...
    if args.distributed and args.rigid_launch:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            mp.spawn(train, args=(args, temp_dir, replay_buffer), nprocs=args.num_gpus)
    else:
        """
