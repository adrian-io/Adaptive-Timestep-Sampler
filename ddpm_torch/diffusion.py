...
def get_beta_schedule(beta_schedule, beta_start, beta_end, timesteps, dtype=torch.float64):
    if beta_schedule == 'quad':
        ...
        for i in range(timesteps):
            t1 = i / timesteps
            t2 = (i + 1) / timesteps
            betas_list.append(min(1 - alpha_bar_fn(t2) / alpha_bar_fn(t1), max_beta))
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        # MODIFIED
        self.loss_coef = betas**2 / (2 * self.fixed_model_var * alphas * (1. - self.alphas_bar))
    ...
    def p_sample_prev(self, denoise_fn, sampled_x_t, sampled_t, generator=None):
        x_t_minus_1 = self.p_sample_step(denoise_fn, sampled_x_t, sampled_t, generator=generator, return_pred=False)
        return x_t_minus_1
    ...
    def train_losses(self, denoise_fn, x_0, t, noise=None): # t: sampled timesteps, denoise_fn : self.model (Unet)
        if noise is None:
            noise = torch.randn_like(x_0)
        ...
        if self.loss_type == "kl":
            ...
            #loss_coef = self._extract(self.loss_coef, t, x_t)
            #losses = loss_coef*((target - model_out).pow(2))
            losses = ((target - model_out).pow(2))
            losses = flat_mean(losses)
